...
`test_api_routes.py` can just ask for `client` as a parameter — same mechanism as `cart` back
in level 4, just not needing an import this time.

**`TestClient`** wraps your FastAPI app in-process — no server, no socket, no real network call
— and gives you `requests`-like methods (`.get()`, `.post()`, ...) that run a request straight
